...
public class AccessCodeService {
    ...
    public AccessCode createAccessCode(Integer courseId, Integer lectureId, Integer userId, String codeValue) { 
        ...
        Course course = courseRepository.findById(courseId)
                             .orElseThrow(() -> new AccessCodeNotFoundException("Course not found for ID: " + courseId)); // استخدام استثناء مخصص

        // 2. Fetch Lecture (optional)
        Lecture lecture = null;
        if (lectureId != null) {
            lecture = lectureRepository.findById(lectureId)
                        .orElseThrow(() -> new AccessCodeNotFoundException("Lecture not found for ID: " + lectureId)); // استخدام استثناء مخصص
        }

        // 3. Fetch User (optional, intended recipient/creator)
        User user = null;
        if (userId != null) {
             user = userRepository.findById(userId).orElseThrow(() -> new AccessCodeNotFoundException("User not found for ID: " + userId)); // استخدام استثناء مخصص
        }
    ...
    // 💡 تم إضافة @Transactional لجعل العملية ككل Transaction واحدة (عادةً ما يساعد في JPA)
    @Transactional 
    public AccessCode redeemAccessCode(String codeValue, Integer userId, Integer lectureId) {
        ...
        // إجبار تحميل العلاقات المرتبطة قبل انتهاء الـ Transaction
        // يضمن أن محول JSON لديه جميع البيانات اللازمة.
